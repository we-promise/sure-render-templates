...
| --- | --- | --- | --- | --- |
| **unit** | `scripts/use-image-tag.sh` rewrites only image url lines. `scripts/update-deploy-branches.sh` builds all 6 branches with the right root `render.yaml` and keeps the AlphaClaw build files only on external-ai branches. Both run in a throwaway clone. | bats, git | $0 | push to main, PRs |
| **contract** | Static invariants over every Blueprint: image tag matches the branch name, `autoDeployTrigger: off`, Postgres basic-1gb on PG 18, starter sizing + Puma 1x3, health check + storage disk, worker runs Sidekiq with web's `SECRET_KEY_BASE`, no dangling env references, no-AI settings blank, SSL settings on. **Drift check**: each live deploy branch equals what the generator would produce from `branches/`. Workflow guard rails and shellcheck. | bats, python3 + PyYAML, shellcheck | $0 | push to main, PRs |
| **local e2e** | Boots a deploy branch's Blueprint in docker compose, generated from that branch's own `render.yaml` (`tests/lib/blueprint.py`). Checks: right image tag; web + worker stay up; DB fully migrated; runs as behind a TLS proxy (plain http gets https links + HSTS, since `RAILS_ASSUME_SSL`/`RAILS_FORCE_SSL` are on); smoke (health, sign up, log in) through the `X-Forwarded-Proto: https` header Render's proxy adds; Sidekiq registered in Redis; a job enqueued by web gets processed by the worker. | docker, bats, curl, python3 + PyYAML | $0 | push to main, PRs |
| **Render e2e** | Creates the Blueprint's resources in Render through the API, at the Blueprint's own plans, named `<name>-e2e-<run>`. Waits for the deploys to go live, runs the same smoke against the real https URL, then deletes everything and verifies nothing with the run's suffix is left. Also runs Render's Blueprint validate API on all 6 files. | `RENDER_API_KEY` secret | **billable**, see below | manual only (`render e2e` workflow) |
...
1. Add the deploy branch to the `e2e-local` matrix in `.github/workflows/test.yml` and to the `branch` choices in `render-e2e.yml`.
...
- Missing prerequisites fail, they don't skip. A skip shows green, and CI must never go green unproven.
