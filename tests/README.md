...
| **local e2e** | Boots a deploy branch's Blueprint in docker compose, generated from that branch's own `render.yaml` (`tests/lib/blueprint.py`). Checks: right image tag; web + worker stay up; DB fully migrated; plain http redirects to https (`RAILS_FORCE_SSL` is live); smoke (health, sign up, log in) through the `X-Forwarded-Proto: https` header Render's proxy adds; Sidekiq registered in Redis; a job enqueued by web gets processed by the worker. | docker, bats, curl, python3 + PyYAML | $0 | push to main, PRs |
